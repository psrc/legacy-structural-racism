...
# 
# old code before `rr = TRUE` was an option

# adjust race labels based on rel. reliability/CV - >30 (use with caution, examine with great caution) mark with *
# dt_2021_test <- dt_2021 %>% 
#   mutate(HRACE=case_when(grepl("(fair|caution|unreliable)",reliability)~paste0(HRACE,"*"),
#                          TRUE~HRACE))
...
dt_2021_simprace <- dt_2021 %>%
  filter(!grepl("(Multiple|Two|Some)",HRACE))
...
# adjust race labels based on rel. reliability/CV - >30 (anything other than 'good' or 'fair') mark with *
dt_2021 <- dt_2021 %>%
...
# reduce number of race/ethnicity categories, if wanted - removing "multiple", "two or more", and "some other" to make easier to understand
dt_2021_simprace <- dt_2021 %>%
  filter(!grepl("(Multiple|Two|Some)",PRACE))
...
# 2019 equity tract data -----
# 2019 is still in the 2010 census tract geographies, but the data is closest to the life expectancy data (2016-2020) - 2020 equity tracts would require the crosswalk, which would add more fuzziness 
equity_tracts_19 <- equity_tracts %>% 
  dplyr::filter(data_year==2019)

# access census population data by tract through ACS to weight life expectancy values - tracts in 2010 geographies but with most recent population numbers (2019)
tracts_19 <- get_acs_recs(geography ='tract',
                          table.names = 'B01003',
                          years = c(2019),
                          acs.type = 'acs5')

# join WTN health data (in 2010 census tracts) to cross walk - we are not using the crosswalk because we are using census tracts from 2010 instead of 2020
...
health_risk_equity_tracts <- merge(life_expec, 
                                   equity_tracts_19,
                                   by.x="Census.Tract",
                                   by.y="geoid", 
                                   all.y =TRUE)

# join 2019 acs population data to WTN health data
health_equity_tract19 <- merge(tracts_19, health_risk_equity_tracts,
                               by.x="GEOID",
                               by.y="Census.Tract",
                               all.y=TRUE)
```
...
health_equity_quintiles19 <- health_equity_tract19 %>% #Warning: NAs introduced by coercion
...
health_equity_pivot19 <- health_equity_quintiles19 %>% 
...
health_equity_pivot19_calc <- health_equity_pivot19 %>%
...
# calculations to get weighted average life expectancy per equity group/quintile by county
# health_equity_wt19 <- health_equity_pivot19 %>% 
...
# combine weighted county and regional values
# health_equity2019 <- rbind(health_equity_pivot19_calc,
#                            health_equity_wt20)
```

```{r}
# renaming labels ----
# storymap is only showing regional values for households of color
health_equity <- health_equity_pivot19_calc %>% 
...
health_equity_poc <- health_equity %>%
  mutate(quintile_ord = fct_relevel(quintile_exp,
